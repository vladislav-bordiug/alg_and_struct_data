...
#в худшем случае она проходит 2 раза по массиву, а на каждом последующем шаге делит его пополам log2(n)+1 раз
#т.е. сложность <= 2n+n+n/2+n/4+...=2n*(1-(1/2)^(log2(n)+1))/(1-1/2)=
#=2n*(1-(1/2)*2^log2((n)^(-1)))/(1-1/2)=2n*(1-1/(2*n))/(1/2)=4n-2
